...
|------|--------------|---------------|
| `announce` | Text-to-speech ([Workers AI](https://developers.cloudflare.com/workers-ai/), Deepgram Aura-2) on chosen rooms or all. The music ducks and carries on afterwards. | Cloud API has no TTS or arbitrary-clip playback for personal integrations |
| `search_sounds` | Finds sound effects in the bridge's library, BBC Sound Effects and (optionally) Freesound, with direct file URLs | Diagnostics |
| `play_sound` | Plays a sound effect (library name or any audio URL; converted on the bridge) over whatever is playing | Same |
...
`play_sound` takes either a name from the bridge's library or a URL. The
bridge downloads the file and converts it with ffmpeg to a normalised mp3,
so wav, ogg, flac and m4a all work, and the speakers only ever fetch from
the LAN. Sonos players silently ignore formats they can't decode, which is
why the conversion isn't optional.

Where sounds come from, in the order Claude is told to try:

1. **The library.** Drop files into `bridge/sounds/` (mounted at
   `/data/sounds`) and play them by filename without the extension.
   `chime` ships with it.
2. **`search_sounds`.** Searches [BBC Sound Effects](https://sound-effects.bbcrewind.co.uk)
   (33,000 clips, no key; personal, educational and research use under the
   BBC RemArc licence) and, if `FREESOUND_API_KEY` is set on the bridge,
   [Freesound](https://freesound.org) (Creative Commons; the licence comes
   back with each result). Results carry direct file URLs for `play_sound`.
3. **The official Sonos connector**, for anything on a music service: songs,
   rain-for-sleep, jingles.
4. A web search, only as a last resort, and only for links to actual audio
   files, not pages that play them.

## Announcements

Speech is generated by the Worker on Workers AI (`TTS_MODEL` and `TTS_VOICE`
in `wrangler.jsonc`; the default is `draco`, a British male voice) and uploaded
to the bridge, which caches it by a hash of model, voice and text. Repeats
aren't generated again and play immediately. No text-to-speech key is needed.
For announcements straight from your LAN (the `announce` script), the bridge
can also call [ElevenLabs](https://elevenlabs.io) if you set `ELEVENLABS_API_KEY`.
...
```bash
cd bridge
cp .env.example .env          # set API_TOKEN (openssl rand -hex 24) and HOST_IP; FREESOUND_API_KEY is optional
...
./announce "Hello" Kitchen    # only if ELEVENLABS_API_KEY is set; Claude's `announce` doesn't need it
```
...
HTTP endpoints are documented at the top of `bridge/app/server.py`. The
bridge's Docker image includes ffmpeg; for the dev server install it locally
(`brew install ffmpeg` / `apt install ffmpeg`), or the conversion tests skip.
